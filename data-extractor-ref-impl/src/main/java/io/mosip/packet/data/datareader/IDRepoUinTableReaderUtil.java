...
public class IDRepoUinTableReaderUtil implements DataReader {
    ...
    private ObjectNode FILTER_PARM;
    ...
    @Value("${mosip.packet.use.filter.param.enabled:false}")
    private boolean isOffsetFilterParamEnabled;
        ...
        if (tableRequestDto.getQueryType().equals(QuerySelection.TABLE)) {
            ...
            if(tableRequestDto.getFilters() != null) {
                for (QueryFilter queryFilter : tableRequestDto.getFilters()) {
                    ...
                    String fromValue = queryFilter.getFromValue();
                    String toValue = queryFilter.getToValue();
                    ...
                    if(isOffsetFilterParamEnabled) {
                        if(queryFilter.getFromValue() == null || queryFilter.getFromValue().isEmpty()) {
                            if(FILTER_PARM.hasNonNull(queryFilter.getFilterField())) {
                                ...
                                fromValue = queryFilter.getInitialFromValue();
                            }
                        }
                    }

                    String condition1 = queryFilter.getFilterField() + " " + queryFilter.getFilterCondition().format(fromValue, toValue, queryFilter.getFieldType());
            ...
            if(tableRequestDto.getExecutionOrderSequence() == 1) {
                ...
                if(!isOffsetFilterParamEnabled)
                    modifiedQuery += " " + QueryOffsetLimitSetter.valueOf(dbType.toString()).getValue(OFFSET_VALUE, ((long) dbReaderMaxThreadPoolCount * dbReaderMaxRecordsCountPerThreadPool));
                else
